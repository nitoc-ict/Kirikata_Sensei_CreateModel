...
model_path = "C:/Users/mi241326/idea/procon/createModel/python3.10.0/model/model2/best_openvino_model"
device = "intel:gpu"
task="detect" # e.g., "detect", "segment", "classify"
...
while cap.isOpened():
    ...
    # YOLOで包丁推論
    yolo_results = yolo_model(frame, device=device)[0] # 推論結果の1つ目
    ...
    # 包丁ボックスとスコアのペアを作る
    knife_boxes_scores = []
    # yolo_resultsがUltralytics標準のResultsオブジェクトか判定
    if hasattr(yolo_results, 'boxes'):
        ...
        for box, cls, score in zip(yolo_results.boxes.xyxy, yolo_results.boxes.cls, yolo_results.boxes.conf):
            if int(cls) == 0:  # 「knife」のクラスID
                xmin, ymin, xmax, ymax = map(int, box)
                knife_boxes_scores.append((score, [xmin, ymin, xmax, ymax]))
    elif isinstance(yolo_results, np.ndarray):
        # OpenVINO推論結果のNumPy配列形式の場合
        # 形状例：(N, 6) でそれぞれ [xmin, ymin, xmax, ymax, conf, cls] の形式を想定
        ...
        raise TypeError("Unknown type for yolo_results")
