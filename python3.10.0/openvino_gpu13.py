...
camera_idx = 0  # 使用するカメラのインデックス

input_length = 640
model_path = "C:/Users/mi241326/idea/procon/createModel/python3.10.0/model/model1/best_openvino_model"
device = "intel:gpu"
task = "detect"
detection_interval = 1 # 何フレーム事に推論を行うか
confidence_threshold = 0.40 # 物体検出の信頼度が50%以上の時、検出成功とする
...
while cap.isOpened():
    ...
    if results and results.multi_hand_landmarks:
        for hand_landmarks, handedness_obj in zip(results.multi_hand_landmarks, results.multi_handedness):
            detected_label = handedness_obj.classification[0].label  # "Left" または "Right"
            if detected_label == "Left": 
                hand_label = "Right"
            else: hand_label = "Left"

            # hand_label = handedness_obj.classification[0].label  # "Left" または "Right"
    ...
    cv2.imshow(f"Knife Safety Detection System camera_idx: {camera_idx}", frame)
    key = cv2.waitKey(1) & 0xFF
    if key == ord('q'): break

    elif key == ord('d') and camera_idx > 0: 
        cap.release()
        camera_idx -= 1
        cap = cv2.VideoCapture(camera_idx)
        if not cap.isOpened(): 
            print(f"カメラ {camera_idx} は利用できません")
            camera_idx += 1
            cap = cv2.VideoCapture(camera_idx)
    elif key == ord('u') :
        cap.release() 
